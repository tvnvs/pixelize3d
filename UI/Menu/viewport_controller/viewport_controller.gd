...
# Event Recieving
func _on_rotation_button_down(angle:float) -> void:
	player_rotation_node.transform = Vector3(0, angle, 0)
	player_rotation_node._send_signal_changed()
	
func _on_top_down_presset_button_down() -> void:
	camera_position_node.transform = Vector3(0, 572.9, 10)
	camera_rotation_node.transform = Vector3(-89, 0, 0)
